...
const RightBox = () => (
	<div
		...
		<div style={{ display: 'flex', flex: 1, marginBottom: 20, background: backgroundOp }} />
		<div style={{ display: 'flex', flex: 2, background: backgroundOp }} />
	</div>
)

const SlideC = ({ showContent }) => {
	return (
		<div style={{ ...containerStyle, background: '#00965b' }}>
...
const circleAnim = {
	animation: {
		...
		willEnter: {
			from: { opacity: 0, scale: 1, x: -150 },
			to: { opacity: 1, scale: 1, x: 0, ease: 'easeOutQuint' }
		},
		willLeave: {
			from: {
				opacity: 1,
...
const rightBoxAnim = {
	animation: {
		...
		willEnter: {
			from: { opacity: 0, x: -400, scaleX: 0 },
			to: { opacity: 1, x: 0, scaleX: 1, ease: 'easeOutQuint' }
		},
		willLeave: {
			from: {
				opacity: 1,
